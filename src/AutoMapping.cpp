...
// MAZE_N_ROWS may differ from MAZE_N_COLS when testing
constexpr uint8_t MAZE_N_ROWS = 9;
constexpr uint8_t MAZE_N_COLS = 9;
constexpr uint8_t NUM_CELLS = MAZE_N_ROWS * MAZE_N_COLS - 12;
...
// Robot position, tracked as it explores.
Coord robot_pos;
...
// Screen is 128x64px == 16x8 tiles. One tile per maze column, one per maze
// row scrolled to keep robot_pos.row in view (MAZE_N_ROWS can exceed the 8
// visible tile rows). The remaining tile columns show a % mapped readout.
constexpr uint8_t MAZE_VIS_TILE_ROWS = 8;
constexpr uint8_t MAZE_VIS_TILE_COLS = MAZE_N_COLS;
...
void draw_maze_visualisation(void) {
    ...
    // Large digits are 2x3 tile cells each; 3 chars ("100") exactly fills
    // the tile columns left of the maze, so no '%' fits there - a small
    // '%' is printed underneath instead.
...
bool drive_to_neighbour(int8_t chosen_dir) {
    ...
    // Double check for a wall in front now that we're facing it.
    auto& robot = GET_ROBOT();
    ...
    robot_drive_straight_with_lidars_no_profile_soft_start(CELL_SIZE_MM, 5000, 100, true, 200);
...
void maze_dfs(void) {
    ...
    while (true) {
        ...
        // matching drive_to_neighbour's delta encoding.
        ...
        if (chosen_dir != -1) {
            ...
            // This path was already driven once (forward), so it should be
            // clear - retry instead of recording a false wall or desyncing
            // `current` from the robot's real position on a failed drive.
...
void shortest_path(const Coord& start, const Coord& end) {
    ...
    // BFS from `end` back to every reachable cell, so following
    // dir_toward_end forward from any cell walks the shortest path to `end`.
    static int8_t dir_toward_end[MAZE_N_ROWS][MAZE_N_COLS];
    for (int i = 0; i < MAZE_N_ROWS; i++) {
        for (int j = 0; j < MAZE_N_COLS; j++) {
            dir_toward_end[i][j] = -1; // not yet reached by the BFS
        }
    }
